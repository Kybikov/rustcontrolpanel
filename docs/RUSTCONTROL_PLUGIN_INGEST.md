...
The global Activity page reads `GET /api/admin/rustcontrol/activity` as an operator investigation log with total/warning/error/operator-note counters, full-text payload search, severity filters, source chips, event-type chips, relative timestamps, backend-resolved server/player entity jump buttons, and compact payload summaries instead of raw JSON-only rows. The same endpoint accepts `actor_id`/`user_id` for Operator Profile history, matching common audit payload fields such as `created_by`, `requested_by`, `updated_by`, `cancelled_by`, `user_id`, `actor_id`, and `started_by`. Operator Profile uses `scope=same_server` on `GET /api/admin/rustcontrol/alerts` so the profile alert block stays personal while Live/Watchlist remain the global alert centers.
...
- Data is stored in `rustcontrol.player_flags`; watch changes, live-player promotion, and operator Steam account binding also write `rustcontrol.activity_events` for Activity/Profile history.
- The Integrations page includes a Rust+ / Plugin Intake panel with public/admin webhook endpoints, `RUSTPLUS_WEBHOOK_SECRET` readiness, accepted source names, realtime traffic checks, source health rows, recent Rust+/plugin events, `Send Test Event`, and `Full Snapshot` actions. The guarded admin/local plugin event form uses the same ingest path as the public webhook after admin auth and HMAC verification.
