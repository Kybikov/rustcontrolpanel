...
- Global Activity now exposes compact `from`/`to` datetime filters in the UI and stores them in saved activity filter presets.
...
- Global Wipe Calendar now supports server/type/range filters, a compact 14-day calendar mode, records mode, upcoming/recent/manual counters, and backend query filtering on `/api/admin/rustcontrol/wipes`.
