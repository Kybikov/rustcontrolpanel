...
func (s *server) watchlistServerHistory(w http.ResponseWriter, r *http.Request) {
	user, ok := s.requireAuth(w, r, "servers.view")
	if !ok {
		return
	}
	...
	history, err := s.checker.History(r.Context(), user.ID, id)
	if errors.Is(err, pgx.ErrNoRows) {
		writeJSON(w, http.StatusNotFound, map[string]string{"error": "saved server not found"})
		return
	}
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "could not load server history"})
		return
	}
